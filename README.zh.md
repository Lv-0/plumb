...
Plumb 会在启动时（每 6 小时至多一次）以及通过菜单栏的「检查更新…」检查更新。检测到新版本时可一键更新——Plumb 会下载更新包、校验其 SHA-256 完整性，然后重开进入一个小型安装器替换 `/Applications/Plumb.app`（因写入 `/Applications` 需要权限，会请求一次密码）。更新完成后应用可能不会自动重启，如需要请手动打开。在 Plumb 获得 Developer ID 签名之前，每次更新后需要重新授予辅助功能 / 屏幕录制权限（见[为什么权限可能需要重新授权](#为什么权限可能需要重新授权以及如何修复)）。
