...
[![Release](https://img.shields.io/badge/release-v1.0.6-success.svg?style=flat-square)](#下载安装)
...
Plumb 会在启动时（每 6 小时至多一次）以及通过菜单栏的「检查更新…」检查更新。检测到新版本时可一键更新——Plumb 会下载更新包、校验其 SHA-256 完整性，然后重开进入一个小型安装器替换 `/Applications/Plumb.app`（因写入 `/Applications` 需要权限，会请求一次密码）。更新后你的辅助功能 / 屏幕录制权限会被保留（见[为什么权限可能需要重新授权](#为什么权限可能需要重新授权以及如何修复)）。
