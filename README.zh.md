...
[![Release](https://img.shields.io/badge/release-v1.0.10-success.svg?style=flat-square)](#下载安装)
...
Plumb 会在启动时（每 6 小时至多一次）以及通过菜单栏的「检查更新…」检查更新。检测到新版本时可一键更新——Plumb 会下载更新包、校验其 SHA-256 完整性，然后重开进入一个小型安装器替换 `/Applications/Plumb.app` 并自动重启应用。如果 app bundle 归你所有（例如从 DMG 拖拽安装），安装器会静默替换、无需密码；否则会请求一次密码。在 Plumb 获得 Developer ID 签名之前，若未创建稳定签名身份，每次更新后需要重新授予辅助功能 / 屏幕录制权限（见[为什么权限可能需要重新授权](#为什么权限可能需要重新授权以及如何修复)）。
