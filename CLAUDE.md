...
- **Settings persist to a signing-independent file** (`~/Library/Application Support/Plumb/settings.json`) as the primary store, with `UserDefaults` (`com.comet.plumb`) as a mirrored secondary. Rationale: `cfprefsd` domains are keyed by the app's code-signing designated requirement, so OTA updates that change the signing identity can make `UserDefaults` read as empty even though the `.plist` still exists on disk. The file path depends only on the bundle-id string, so settings survive across updates regardless of signing. `load()` reads file → falls back to `UserDefaults` (with one-time migration to file) → `.default`. `save()` writes both.
- Document apps (Pages/Numbers/Word/Excel, configurable via `documentChooserBundleIDs`) show a template/file picker before the real document. All three window kinds (picker, unsaved new doc, opened saved doc) share `AXStandardWindow` subrole, so subrole can't distinguish them. Detection uses **only** `kAXDocumentAttribute`: a window with empty `kAXDocument` is treated as non-document (picker or unsaved new doc) → centered-only; a window with a `file://` document is tiled. A title-matching approach was tried and abandoned (AXTitle has a fill-timing race at the 0.45s retry, and picker titles differ across Office vs iWork and locales). Critically, the center-only path **does not** call `markCentered` and **does not** lock `processedPIDs` — otherwise a subsequent real document window (or the same window after save) would be permanently suppressed and never tiled. Trade-off: unsaved new docs are centered-only until saved (acceptable vs the broken state of pickers being tiled and docs not tiled).
