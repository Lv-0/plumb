...
- Document apps (Pages/Numbers/Word/Excel, configurable via `documentChooserBundleIDs`) show a template/file picker before the real document. All three window kinds (picker, unsaved new doc, opened saved doc) share `AXStandardWindow` subrole, so subrole can't distinguish them. The picker is detected by `isKnownTemplateChooserWindow`: empty `kAXDocumentAttribute` **plus** a title matching known chooser titles (currently zh "打开新的和最近使用的文件"). The picker is centered-only and **does not** lock `processedPIDs`, so the subsequent document window still gets tiled. Note: `kAXDocument` alone is insufficient — unsaved new docs also lack it (no file path yet), so title matching is required to avoid mis-tiling new docs as choosers.
